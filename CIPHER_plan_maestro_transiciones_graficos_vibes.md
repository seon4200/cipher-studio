...
| gravedad | qué |
|---|---|
| **BAJA** | **`fuente_recortada.mp4` se queda en disco y nadie lo borra.** Al recortar el vídeo antes de transcribir se materializa una copia en `materiales/originales/fuente_recortada.mp4`, junto al original del usuario. Con un vídeo largo son **cientos de MB** por proyecto: 26 MB para un recorte de 9:40 sobre un fuente de 45 MB, medido, pero un fuente de 2 GB deja un recorte proporcional. Hoy solo se sobrescribe al volver a recortar; nada lo limpia. **Hay que decidir entre dos opciones y no son equivalentes:** borrarlo al cerrar el proyecto libera espacio pero **rompe el proyecto** —la biblioteca apunta a ese fichero, no al original—, así que solo valdría si al cerrar se devolviera la biblioteca al original y se marcara el recorte para rehacerlo al abrir; o conservarlo, que es lo que hace hoy, y aceptar el coste a cambio de que el proyecto siga funcionando sin el fichero del usuario. La segunda es además lo que permite la idea de **duplicar el proyecto con otro recorte** en vez de un "rehacer" destructivo. Ojo: el original se conserva en `pathOriginal`, así que ninguna de las dos pierde material. |
| **MEDIA** | **Los primarios y la curva de color se siguen heredando de la fuente.** La normalización ya fuerza `-colorspace bt709`, que unifica **la matriz** y con eso cierra el bug que descartaba todos los gráficos del export (medido: 30/30 fallos → 0/30). Pero `color_primaries` y `color_transfer` **no** se tocan: un clip que entra `bt2020/arib-std-b67` sale `bt709/bt2020/arib-std-b67`, con la matriz corregida y las otras dos etiquetas de la fuente. El fichero exportado declara `bt709/bt709/bt709` a nivel de stream mientras por dentro alterna la curva, así que **un clip HDR de verdad quedaría lavado**: los reproductores aplican la etiqueta del stream a todo y mostrarían un tramo HLG como si fuera SDR. **Con los dos clips que hay hoy no pasa, y está medido:** `clip_006` y `clip_013` llevan etiqueta HDR sin serlo —el rango p10-p90 de luminancia es **idéntico** en las tres variantes (59 y 99 respectivamente), así que no hay contraste que recuperar. La solución completa es tonemapping HLG→SDR con `zscale`+`tonemap`, medido en **+70% de tiempo** (430→724 ms por clip, ~0.6s sobre una normalización de 35s); se descartó porque **`zscale` no está en todas las builds de ffmpeg** y arreglaría algo que hoy no se ve. Rehacerlo el día que entre stock HDR real. |
| **ALTA — ANTES DE LANZAR** | **SEIS CLAVES DE API EN EL HISTORIAL DE GITHUB.** `.env` estuvo trackeado con valores reales de **DeepSeek, ElevenLabs, fal.ai, Pexels, Pixabay y Coverr**. Están en los commits **`167097a`, `834bc43` y `2a7c4ce`**, y los tres son antepasados de `origin/master` (`66f4c5b`), o sea que **ya están publicados** en `github.com/seon4200/cipher-studio`. Se recuperan con un simple `git show 167097a:.env`. **Quitar `.env` del índice —que es lo que hizo `2a7c4ce`— NO las quitó del historial**: solo dejó de trackear el fichero. **ORDEN OBLIGATORIO: rotar las seis PRIMERO, reescribir el historial DESPUÉS.** Limpiar sin rotar no sirve de nada: si el repo estuvo público hay que asumir que están copiadas, y ninguna reescritura vuelve segura una clave ya expuesta. La reescritura (`git filter-repo` o BFG) sirve para no volver a publicarlas, no para recuperarlas. |
| **ALTA — ANTES DE LANZAR** | **El repo pesa 218 MB porque `node_modules` estuvo commiteado**, incluido `electron.exe` de **172 MB**, más `app-builder-bin` para cinco plataformas y un `project-state.json` que llegó a 12.16 MB. Hoy no queda ni un fichero de `node_modules` trackeado —se limpió— pero **sigue todo en los objetos**: un clon se descarga 218 MB para 206 ficheros de código. **La misma reescritura del historial que saca el `.env` lo arregla**, así que conviene hacer las dos cosas en la misma pasada y no dos veces. |
| **ALTA** | **PÉRDIDA DE DATOS SILENCIOSA AL CERRAR LA APP.** El proyecto `fgdgf` (28 min, 818 clips, `project-state.json` de **107 MB**) perdió al cerrar la app el `aiScript`, el `originalTranscriptText`, los **726 `transcriptSegments`** y los **148 clips de gráfico**. Nada avisó: el fichero se escribió "con éxito". **Es un fichero a medias, no un guardado vacío** — y es más grave cuanto mayor es el proyecto. Dos mecanismos, y hay que mirar los dos: **(1)** el manejador de cierre (`index.ts:107-123`) hace `preventDefault`, manda `save-before-close` y **destruye la ventana a los 4 s pase lo que pase**; serializar y escribir 107 MB puede tardar más, y entonces el renderer muere a mitad. **(2) `save-project-state` (`index.ts:1214`) escribe con `fs.promises.writeFile` DIRECTAMENTE sobre el fichero real — sin temporal + rename — así que una interrupción a mitad de escritura TRUNCA el original**; verificado leyendo el código, no es una hipótesis. La salida es escribir a `.tmp` y renombrar (el rename es atómico) y que el cierre espere al guardado en vez de a un cronómetro. Hoy se salvó **por accidente**: el guion se pudo reconstruir porque `regenerate-graphics` deja el crudo de Whisper en `materiales/audio/transcripts/` y no lo borra, al contrario que los otros dos caminos de transcripción. No hay ningún backup: el `project-state.json` de la raíz está vacío y no es una copia — sale de `activeProjectPath \|\| process.cwd()` en el mismo handler. **EMPEORADO desde que la transcripción guarda `words`:** medido, **145 bytes por palabra**, que en un proyecto de 423 segmentos son **~2.2 MB más** sobre los 13.4 MB actuales (**+17%**). Agrandar el fichero **no crea** el bug, pero **ensancha la ventana** en la que puede morder: más bytes que serializar y escribir es más tiempo dentro del cronómetro de 4 s del cierre, y más tiempo con el fichero real a medio reescribir. |
| ~~ALTA~~ **✅ RESUELTA** (sin commit todavía) | **Vía 4** — `generate-perfect-sync` dejaba el **`audioClip`** apuntando fuera del proyecto. El frontend le pasa `audioClipExisting?.path \|\| firstVideo.path` (`main.tsx:3892`): si ya se pulsó "Usar Audio Original" llega `materiales/audio/maestro.m4a` y todo está bien, pero **si no, cae al `.mp4` del usuario** y el clip de tipo `audio` acaba siendo el vídeo entero. Era el bug que cerró M1 por una tercera puerta. **Arreglado extrayendo a `materiales/audio/maestro.m4a`.** El ffmpeg de M1 se sacó a `extraerAudioMaestro(videoPath, projPath)` con **dos llamadores** —el botón y la sincronía perfecta— en vez de duplicarlo, que es justo el error que M1 documentó con los dos botones. Si la extracción falla **no se aborta** (a esas alturas ya se gastó dinero en clips de IA): se sigue con el audio externo y se devuelve `audioExterno: true` para que PIEZA A lo avise. **Verificado sobre el bundle compilado**, tres casos: sin audio previo → extrae dentro; con audio ya dentro → lo reutiliza sin re-extraer (mtime intacto); un directorio hermano `<proyecto>-copia` → se trata como fuera (por eso `dentroDelProyecto` usa `path.relative` y no `startsWith`). |
| — | ~~El `v1Clip` apunta fuera~~ — **no es deuda, es la decisión cerrada** *"no copiar el vídeo importado"*. La redacción anterior de la Vía 4 mezclaba las dos mitades y hacía parecer que había que copiarlo. El `v1Clip` se queda fuera **a propósito** y su cobertura es **PIEZA A** (avisar), no la Vía 4. |
...
| **MEDIA** | **Las rampas de `AnimatedGraphic` son segundos absolutos, no fracción de la duración** — un gráfico más corto que su rampa congela el valor a mitad y **eso se graba**. El `contador` en 1 s se queda al 66.7%: un 87 sale como **58**, un dato falso en el vídeo. Hoy está **latente** porque DeepSeek viene devolviendo 2.0 s, no porque el código lo impida. Ningún test lo detecta. Detalle medido, tabla por tipo y por qué el arreglo espera a los templates: **[docs/deuda-graficos.md](docs/deuda-graficos.md)**. |
| **BAJA** | **La barra de progreso va una unidad por delante en FASE 3.** El frontend hace `current: data.index + 1` (`main.tsx:1403`), correcto para FASE 2 que manda `index: batchStart` (base 0), pero FASE 3 manda `index: clipNum` **ya en base 1** (`index.ts:3698`) → muestra `N+1 de N` al terminar. Dos de los tres paneles no acotan el ancho (`main.tsx:3714` y `:5923`; solo `:3871` tiene `Math.min`), así que la barra se pasa de largo. Cosmético, detectado al diagnosticar la Vía 4. |
| **BAJA** | **`generate-perfect-sync` tiene un fallback que escribe fuera del proyecto**: si no llega `projPath`, los v2Clips van a `banco-clips/sync-perfecta` (`index.ts:3663`). Hoy el frontend siempre manda `activeProjectPath`, así que no se dispara — queda anotado por si alguna vez se llama al handler sin proyecto. |
| **BAJA** | **`newAudioSegments` en el clip de audio es un campo muerto.** `usarAudioOriginal` lo escribe (`main.tsx:976`) y **nadie lo lee jamás**: `handleBuildTimeline` decide por el **nombre** del clip (`voiceClip?.name === 'Voz - Audio Original'`, `main.tsx:2326`) y saca los segmentos del estado de nivel superior. Medido: cero lecturas de `clip.newAudioSegments` en todo `src/`. **Por eso que sincronía perfecta no lo propague no es un bug** — se comprobó al cerrar la Vía 4. **Quitar la escritura de `main.tsx:976` toca cuando se ataque el tamaño del `project-state.json` (deuda MEDIA), no antes**: hoy solo duplica datos dentro de `timelineVideoClips`, y sacarlo suelto sería un cambio sin verificación propia. |
| **MEDIA** | **La cola sin transcribir produce fotograma congelado al final, y ya no es culpa del modelo.** Whisper siempre deja algo de audio sin cubrir al final; FASE 5 estira el último clip para tapar ese hueco y el `tpad` lo rellena clonando el último fotograma. **Segunda vez medido: 9.00 s congelados** en un vídeo de 28 min (`fgdgf`, hueco de 10 s con 1 s de metraje), tras los 3.11 s del proyecto anterior. **El aviso B1 no salta por un segundo**: su umbral es `MAX_CLONADO = 10 s` y esto son 9.00. Y con `base` la voz perdida en ese audio es del **0.1 %**, así que **cambiar de modelo ya no lo arregla** — el problema no es lo que Whisper se deja de oír, sino que **nada cubre esa cola con metraje real**. Las salidas serían bajar el umbral del aviso, o rellenar la cola con imagen de verdad en vez de un fotograma quieto. Ninguna decidida. |
| **MEDIA** | **El modelo de Whisper debería ser configurable desde ajustes**, con `base` por defecto y `tiny` disponible para equipos lentos. `MODELO_WHISPER` ya está extraída a una constante (`index.ts`), así que **el backend está hecho: es trabajo de interfaz**. Lo que hay que enseñarle al usuario es el precio real de elegir `tiny`, medido sobre dos audios: **pierde entre el 6.5 % y el 11.5 % de la narración** sin transcribir. Y no es solo un subtítulo incompleto — **el guion, los cortes y los `graphicStart` se calculan sobre ese texto**, así que cada segundo perdido es un tramo del vídeo que se monta a ciegas: sin frase donde anclar un gráfico y con clips colocados por estiramiento en vez de por tiempo de frase. Un desplegable sin esa advertencia invita a elegir mal. |
| **BAJA** | **`dist/index.html` está trackeado pese a que `dist/` está en `.gitignore`** — se commiteó antes de ignorarlo, y `.gitignore` no afecta a lo ya trackeado. Cada `npm run build` cambia el hash del bundle (`principal-XXXX.js`) y ensucia el árbol, así que siempre hay un fichero modificado que no es tuyo. Se cierra con `git rm --cached dist/index.html`, pero **eso cambia lo que se publica** y no toca decidirlo en mitad de otra cosa. |
| **BAJA** | **La cola del `.gitignore` está en UTF-16LE.** Tras el bloque bueno hay tres patrones repetidos —`node_modules/`, `dist/`, `dist-electron/`— escritos en UTF-16 y leídos como UTF-8: se ven con un espacio entre cada letra, que es el byte nulo. **Es el destrozo de codificación de PowerShell que la regla 7 existe para evitar, fosilizado en el repo.** Hoy es inofensivo porque esos tres patrones ya están arriba y git lee las líneas corruptas como basura que no casa con nada — pero **una regla nueva añadida ahí abajo no funcionaría**, y el fallo sería mudo. |
