...
ipcMain.handle('generate-timeline-assets', async (event, { scriptText }) => {
  try {
    ...
    // 2. Leer directrices visuales de creative-unlock.md
    ...
    // 4. Prompt a DeepSeek para analizar y clasificar cada párrafo
    const prompt = `
    ...
    for (let idx = 0; idx < analysisResults.length; idx++) {
      ...
      if (type === 'remotion') {
        ...
        await new Promise<void>((resolve, reject) => {
          ...
          exec(cmd, { cwd: remotionProjectRoot }, (err, _stdout, stderr) => {
    ...
    <script>
      ...
      window.__timelines["main"] = tl;
        ...
        await new Promise<void>((resolve, reject) => {
          ...
          exec(cmd, { cwd: hyperframesProjectRoot }, (err, _stdout, stderr) => {
            try { fs.unlinkSync(compositionHtmlPath); } catch (e) {}
            if (err) {
